...
function GenresTableItem({ genre, onRefresh }) {
    const handleDelete = async () => {
        try {
            await ApiRequest.delete(`/genres/${genre.id}`)
            onRefresh()
        } catch (error) {
            console.log(error)
        }
    ...
    const handleRestore = async () => {
        try {
            await ApiRequest.patch(`/genres/${genre.id}/restore`)
            onRefresh()
        } catch (error) {
            console.log(error)
        }
    ...
    return (
        <tr className="hover:bg-slate-50 dark:hover:bg-slate-700/25">
            <td className="px-4 py-3 whitespace-nowrap">
                <div className="text-left">{genre.name}</div>
            </td>
