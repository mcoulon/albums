...
function MediaTypesSubmit() {
    ...
    useEffect(() => {
        if (id) {
            ApiRequest.get(`/media-types/${id}`)
                .then((response) => {
                    setMediaType(response.data.data)
                })
                .catch((error) => {
                    console.log(error)
                })
    ...
    const onSubmit = async (data) => {
        try {
            if (id) {
                await ApiRequest.put(`/media-types/${id}`, data)
            } else {
                await ApiRequest.post(`/media-types`, data)
            }
            navigate('/mediatypes')
        } catch (error) {
            console.log(error)
            ...
            <div className="w-[90%] max-w-md">
                <div className="bg-white p-8 rounded-xl shadow-lg">
                    <h2 className="text-2xl font-bold text-gray-800 mb-6">
                        {id ? 'Modifier un type de média' : 'Ajouter un type de média'}
                    </h2>
                    <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    if (id) {
                                        setMediaType({ ...mediaType, name: e.target.value })
                                    }
                                }}
                            />
...
export default MediaTypesSubmit
