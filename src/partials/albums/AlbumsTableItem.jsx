...
function AlbumsTableItem({ album, onRefresh }) {
    const handleDelete = async () => {
        try {
            await ApiRequest.delete(`/albums/${album.id}`)
            onRefresh()
        } catch (error) {
            console.log(error)
        }
    ...
    const handleRestore = async () => {
        try {
            await ApiRequest.patch(`/albums/${album.id}/restore`)
            onRefresh()
        } catch (error) {
            console.log(error)
        }
    ...
    return (
        <tr className="hover:bg-slate-50 dark:hover:bg-slate-700/25">
            <td className="px-4 py-3 whitespace-nowrap">
                <div className="text-left">{album.title}</div>
            </td>
            <td className="px-4 py-3 whitespace-nowrap">
                <div className="text-left">{album.artist?.name}</div>
            </td>
